...
--[[
_process_rule - 执行规则(重要函数)

参数：
	self
	rule - 单个规则。数据结构示例参见rules目录下的json文件。
	collections - http参数集
	ctx - waf上下文

rule(json)示例：以下rule表示 -
拦截(actions) 所有header的pragma字段(vars) 等于(operator) "no-cache"(pattern) 的请求
{
	"actions" : {
		"disrupt" : "CHAIN"
	},
	...
	for k, v in ipairs(rule.vars) do
		...
		if not collection then
			...
			if match then
				...
				-- 对rule的collection需要做记录
				-- 这一点也比较重要，要打到日志里，可以用于rule验证
...
-- main entry point
--[[
1. 执行阶段：这个函数每个用到的阶段都会调用，因此需要在内部通过phase来区分逻辑。
2. 关键变量和参数：ctx、self、collections、opts
	ctx - waf context对象，用于管理单个阶段的所有waf上下文，包括phase、collections、log等
	self - 存储一些全局的变量，例如: 规则集self._active_rulesets
	collections - http请求上下文
]]
function _M.exec(self, opts)
	...
	-- 2. ctx和collections变量的初始化
	-- ctx优先从ngx.ctx.lua_resty_waf中获取，显然lua_resty_waf可以用作在请求的各阶段共享waf数据。
	...
	for _, ruleset in ipairs(self._active_rulesets) do
		...
		-- 参照rules目录下的json文件，每个阶段的rule集是1个数组。
		-- offset 用于跳转到下一个rule
		local offset = 1
